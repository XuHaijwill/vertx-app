...
/**
 * Main Verticle Ôø?HTTP server entry point.
 * <p>
 * Responsibilities:
 ...
 * - Start HTTP server
 * <p>
 * All business logic lives in com.example.api.* Ôø?add new API modules here.
 */
public class MainVerticle extends AbstractVerticle {
    ...
    private Future<Void> deployDatabaseVerticle() {
        ...
        vertx.deployVerticle("com.example.db.DatabaseVerticle",
                ...
                .onFailure(err -> {
                    LOG.warn("[WARN] DatabaseVerticle failed Ôø?demo mode: {}", err.getMessage());
                    p.complete();
                });
        return p.future();
    }
    ...
    private Future<Void> deploySchedulerVerticle() {
        ...
        vertx.deployVerticle("com.example.verticles.SchedulerVerticle",
                ...
                .onFailure(err -> {
                    LOG.warn("[WARN] SchedulerVerticle failed Ôø?continuing: {}", err.getMessage());
                    p.complete();
                });
        return p.future();
    }
    ...
    private Future<KeycloakAuthHandler> setupAuth(Router router, String contextPath) {
        ...
        if (!authConfig.isEnabled()) {
            LOG.info("[AUTH] Authentication disabled Ôø?all endpoints are open");
            return Future.succeededFuture(null);
        }

        LOG.info("[AUTH] Authentication enabled Ôø?issuer={}, clientId={}",
                authConfig.getIssuer(), authConfig.getClientId());

        // Paths that skip authentication (Âä†‰∏ä context-path ÂâçÁºÄ)
        Set<String> skipPaths = new HashSet<>(java.util.Arrays.asList(
                ...
                // User API ÁôΩÂêçÂçïÔºàÊñπ‰æøÊµãËØïÔø?            contextPath + "/api/users",
    ...
    private void attemptListen(Router router, int port, int attempts, Promise<Integer> p) {
        vertx.createHttpServer().requestHandler(router).listen(port)
                ...
                .onFailure(err -> {
                    ...
                    if (bind && attempts < 10) {
                        LOG.warn("Port {} in use Ôø?trying {}", port, port + 1);
